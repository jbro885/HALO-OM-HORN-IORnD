...
if(buildInt == 8){
 if (seq[0] == '[') {
      switch (seq[1]) {
      case 'A': printf("A \n"); 
      myChara = 132; return 129; //'w';
        case 'B': printf("B \n");
         myChara = 131; return 130; // 's';
        case 'C': printf("C \n"); 
        myChara = 130; return 131; // 'd';
        case 'D': printf("D \n"); 
        myChara = 129; return 132; // 'a';
      }
    }
...
//replace(static_list, c);
 //converttobinary(c, op, sizeof(op)); 
   converttobinaryMalloc(myChara, opMalloc, 32); 
       // printf("binary : %s\n", opMalloc);
//printf("%s\n", growing_list);
...
write2Position(stdKbdInOffset_H , buffer0, buffer1 , opMalloc , strlen(opMalloc), stdBlockSize_H);
//printf("buffer1= %s \n",buffer1);
