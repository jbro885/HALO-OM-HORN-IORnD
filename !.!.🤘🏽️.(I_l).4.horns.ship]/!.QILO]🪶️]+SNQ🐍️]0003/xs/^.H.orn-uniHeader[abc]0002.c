...
//char bin_mt;
   ...
   printf("The time is: %02d:%02d:%02d:%02d:%02d:%02d\n",ptm->tm_year ,ptm->tm_mon,ptm->tm_mday ,  ptm->tm_hour,
      ptm->tm_min, ptm->tm_sec);
       ...
       dec_year = ptm->tm_year ; 
 dec_mon = ptm->tm_mon;
 dec_mday = ptm->tm_mday;
 dec_hour =  ptm->tm_hour ; 
 dec_min = ptm->tm_min ; 
 dec_sec = ptm->tm_sec ; 
 
// char *bin_mt = malloc(8);
converttobinary(dec_year, bin_year, 8); 

strcat(strcat(bintime, bin_year), ":");



converttobinary(dec_mon, bin_mon, 8); 
strcat(strcat(bintime, bin_mon), ":");
converttobinary(dec_mday, bin_mday, 8); 
strcat(strcat(bintime, bin_mday), ":");

converttobinary(dec_hour, bin_hour, 8); 
strcat(strcat(bintime, bin_hour), ":");

converttobinary(dec_min, bin_min, 8); 


strcat(strcat(bintime, bin_min), ":");
converttobinary(dec_sec, bin_sec, 8); 
//strcat(strcat(bintime, bin_sec), ":");

//bin_year = bin_mt 
//free(bin_mt);

// i dont want mem leak every time i convert  2 binary 
/*
converttobinary(dec_mon, bin_mon, 8); 
converttobinary(dec_mday, bin_mday, 8); 
converttobinary(dec_hour, bin_hour, 8); 
...
converttobinary(dec_sec, bin_sec, 8); 
*/
//snprintf(bintime,48, "%s:%s:%s:%s:%s:%s\n",bin_year,bin_mon,bin_mday,bin_hour,bin_min,bin_sec);

//strcpy(strcpy(strcpy(strcpy(strcpy(bintime, bin_year), ": "),
      //  bin_mon), ": "), bin_mday);
      
    //  strcat(bintime, bin_year);
 //  strcat(strcat(bintime, bin_year), ":");
  //  strcpy(bintime, bin_mon);
   // strcpy(bintime, ":");
...
//sprintf(bintime, "%s:%s:%s:%s:%s:%s\n",bin_year,bin_mon,bin_mday,bin_hour,bin_min,bin_sec);
     ...
     /*   sprintf(bintime, "%02d:%02d:%02d:%02d:%02d:%02d\n",ptm->tm_year ,ptm->tm_mon,ptm->tm_mday ,  ptm->tm_hour,
      ptm->tm_min, ptm->tm_sec);
      */
      
      /*
      sprintf(bintime, "%s:%s:%s:%s:%s:%s\n",converttobinary(
      ptm->tm_year, bin_mt, sizeof(bin_mt)),
      converttobinary(ptm->tm_mon, bin_mt, sizeof(bin_mt)),
      converttobinary(ptm->tm_mday, bin_mt, sizeof(bin_mt)),
      converttobinary(ptm->tm_hour, bin_mt, sizeof(bin_mt)),
       converttobinary(ptm->tm_min, bin_mt, sizeof(bin_mt)),
        converttobinary(ptm->tm_sec, bin_mt, sizeof(bin_mt)),
      );
      */
