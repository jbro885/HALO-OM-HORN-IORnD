...
mov %rbx , %r15
 ...
 drop_finder_2:
 #🚿️APR13 - this is working fine now. sub after nand <3
 #u dont actually have 2 sub at all cuz it wont make a diff
 # on "FINAL OUTPUT", so lets hold off for a minute 2 not overcompl
 #🚿️🚿️🚿️🚿️🚿️🚿️🚿️🚿️🚿️🚿️🚿️🚿️🚿️🚿️
 compare_r14_2: 
 cmp  $'_' , %r14 #auto fill (if availiable)
 		#throw error message if not
 jne default_fx_2
 #has 2 be "redone?" 2 include "%r15" (ie second var input)

sub $1 ,%r10 #to blank #not subbing yet
mov (%r13,%r10,1) , %r14 
add $1 ,%r10
compare_r15_2: 
 cmp  $'_' , %r15
...
#sub $1 ,%r10 #no sub or add (cuz alredy in pos...)
mov (%r13,%r10,1) , %r15
#add $1 ,%r10
